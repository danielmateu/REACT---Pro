import {
  __toESM,
  require_react
} from "./chunk-OMFGQDFA.js";

// node_modules/dmp-product-card/dist/dmp-product-card.esm.js
var import_react = __toESM(require_react());
var useProduct = function useProduct2(_ref) {
  var onChange = _ref.onChange, product = _ref.product, _ref$value = _ref.value, value = _ref$value === void 0 ? 0 : _ref$value, initialValues = _ref.initialValues;
  var _useState = (0, import_react.useState)((initialValues == null ? void 0 : initialValues.count) || value), counter = _useState[0], setCounter = _useState[1];
  var isMounted = (0, import_react.useRef)(false);
  var maxCount = initialValues == null ? void 0 : initialValues.maxCount;
  var handleAdd = function handleAdd2(value2) {
    var newValue = Math.max(counter + value2, 0);
    if (maxCount) {
      newValue = Math.min(newValue, maxCount);
    }
    setCounter(newValue);
    onChange && onChange({
      count: newValue,
      product
    });
  };
  var reset = function reset2() {
    setCounter((initialValues == null ? void 0 : initialValues.count) || value);
  };
  (0, import_react.useEffect)(function() {
    if (!isMounted.current)
      return;
    setCounter((initialValues == null ? void 0 : initialValues.count) || value);
  }, [initialValues == null ? void 0 : initialValues.count, value]);
  (0, import_react.useEffect)(function() {
    isMounted.current = true;
  }, []);
  return {
    counter,
    isMaxCountReached: !!(initialValues != null && initialValues.count) && initialValues.maxCount === counter,
    maxCount: initialValues == null ? void 0 : initialValues.maxCount,
    handleAdd,
    reset
  };
};
function styleInject(css, ref) {
  if (ref === void 0)
    ref = {};
  var insertAt = ref.insertAt;
  if (!css || typeof document === "undefined") {
    return;
  }
  var head = document.head || document.getElementsByTagName("head")[0];
  var style = document.createElement("style");
  style.type = "text/css";
  if (insertAt === "top") {
    if (head.firstChild) {
      head.insertBefore(style, head.firstChild);
    } else {
      head.appendChild(style);
    }
  } else {
    head.appendChild(style);
  }
  if (style.styleSheet) {
    style.styleSheet.cssText = css;
  } else {
    style.appendChild(document.createTextNode(css));
  }
}
var css_248z = "\n\n.styles-module_productCard__oaIVo {\n    background-color: #1E2025;\n    border-radius: 15px;\n    box-shadow: 0px 2px 10px rgba(0,0,0,0.15);\n    color: white;\n    padding-bottom: 5px;\n    font-family: Arial, Helvetica, sans-serif;\n    width: 250px;\n    margin-right: 5px;\n    margin-top: 5px;\n}\n\n.styles-module_productImg__vPsTp {\n    border-radius: 15px 15px 0px 0px;\n    width: 100%;\n}\n\n.styles-module_productDescription__Ariub {\n    margin: 10px;\n}\n\n.styles-module_buttonsContainer__ghCDF {\n    margin: 10px;\n    display: flex;\n    flex-direction: row;\n}\n\n.styles-module_buttonMinus__BTgTf {\n    cursor: pointer;\n    background-color: transparent;\n    border: 1px solid white;\n    border-radius: 5px 0px 0px 5px;\n    color: white;\n    font-size: 20px;\n    width: 30px;\n}\n\n.styles-module_buttonMinus__BTgTf:hover {\n    background-color: rgba(0, 0, 0, 0.1);\n}\n\n.styles-module_countLabel__S6HZ- {\n    border-bottom: 1px solid white;\n    border-top: 1px solid white;\n    color: white;\n    font-size: 16px;\n    /* height: 25px; */\n    padding-top: 5px;\n    text-align: center;\n    width: 30px;\n}\n\n.styles-module_buttonAdd__s8wd6 {\n    cursor: pointer;\n    background-color: transparent;\n    border: 1px solid white;\n    border-radius: 0px 5px 5px 0px;\n    color: white;\n    font-size: 20px;\n    width: 30px;\n}\n\n.styles-module_buttonAdd__s8wd6:hover {\n    background-color: rgba(0, 0, 0, 0.1);\n}\n\n\n.styles-module_disabled__k5aZm {\n    border-color: grey !important;\n    border-left: 1px solid white !important;\n    color: grey !important;\n}";
var styles = { "productCard": "styles-module_productCard__oaIVo", "productImg": "styles-module_productImg__vPsTp", "productDescription": "styles-module_productDescription__Ariub", "buttonsContainer": "styles-module_buttonsContainer__ghCDF", "buttonMinus": "styles-module_buttonMinus__BTgTf", "countLabel": "styles-module_countLabel__S6HZ-", "buttonAdd": "styles-module_buttonAdd__s8wd6", "disabled": "styles-module_disabled__k5aZm" };
styleInject(css_248z);
var ProductContext = (0, import_react.createContext)({});
var Provider = ProductContext.Provider;
var ProductCard = function ProductCard2(_ref) {
  var children = _ref.children, product = _ref.product, className = _ref.className, style = _ref.style, onChange = _ref.onChange, value = _ref.value, initialValues = _ref.initialValues;
  var _useProduct = useProduct({
    onChange,
    product,
    value,
    initialValues
  }), counter = _useProduct.counter, handleAdd = _useProduct.handleAdd, maxCount = _useProduct.maxCount, reset = _useProduct.reset, isMaxCountReached = _useProduct.isMaxCountReached;
  return import_react.default.createElement(Provider, {
    value: {
      product,
      counter,
      handleAdd,
      maxCount
    }
  }, import_react.default.createElement("div", {
    className: styles.productCard + " " + className,
    style
  }, children({
    count: counter,
    isMaxCountReached,
    maxCount: (initialValues == null ? void 0 : initialValues.maxCount) || 10,
    product,
    handleAdd,
    reset
  })));
};
var ProductButtons = function ProductButtons2(_ref) {
  var className = _ref.className, style = _ref.style;
  var _useContext = (0, import_react.useContext)(ProductContext), counter = _useContext.counter, handleAdd = _useContext.handleAdd, maxCount = _useContext.maxCount;
  var isMaxReached = function isMaxReached2() {
    return !!maxCount && counter === maxCount;
  };
  return import_react.default.createElement("div", {
    style,
    className: styles.buttonsContainer + " " + className
  }, import_react.default.createElement("button", {
    className: styles.buttonMinus,
    onClick: function onClick() {
      return handleAdd(-1);
    }
  }, "-"), import_react.default.createElement("div", {
    className: styles.countLabel
  }, counter), import_react.default.createElement("button", {
    className: styles.buttonAdd + " " + (isMaxReached() ? styles.disabled : ""),
    onClick: function onClick() {
      return handleAdd(1);
    }
  }, "+"));
};
var img = "data:image/jpeg;base64,";
var ProductImage = function ProductImage2(_ref) {
  var img$1 = _ref.img, className = _ref.className, style = _ref.style;
  var _useContext = (0, import_react.useContext)(ProductContext), product = _useContext.product;
  var imgToShow;
  imgToShow = img$1 ? img$1 : product.img ? product.img : img;
  return import_react.default.createElement("img", {
    style,
    src: imgToShow,
    className: styles.productImg + " " + className,
    alt: "Product image"
  });
};
var ProductTitle = function ProductTitle2(_ref) {
  var title = _ref.title, className = _ref.className, style = _ref.style;
  var _useContext = (0, import_react.useContext)(ProductContext), product = _useContext.product;
  var titleToShow = title ? title : product.title ? product.title : "No title";
  return import_react.default.createElement("span", {
    style,
    className: styles.productDescription + " " + className
  }, titleToShow);
};
var ProductCard$1 = Object.assign(ProductCard, {
  Buttons: ProductButtons,
  Image: ProductImage,
  Title: ProductTitle
});
export {
  ProductButtons,
  ProductCard$1 as ProductCard,
  ProductImage,
  ProductTitle
};
//# sourceMappingURL=dmp-product-card.js.map
